...
function displayOffers(offers) {
    ...
    for (var i = 0; i < offers.length; i++) {
        var offer = offers[i];
        if (offer.id_person === sessionStorage.getItem('found_id')) {
            ...
            contentElement.innerText = `Offer's short description: \n ${offer.content} `;
...
function goEdit() {
    // Implement the functionality for editing an offer here
}
...
function confirmAction(confirmation) {
    ...
    if (confirmation) {
        var encodedSelectedOffer = encodeURIComponent(selected_offer);
...
function validateOffer(arg2, arg3, arg4, arg5, result) {
    ...
    if (!title.trim() || !content.trim() || !info.trim() || !salary.trim()) {
        ...
        xhttp.open("GET", "offerAddingServlet?arg1=" + sessionStorage.getItem('found_id') + "&arg2=" + title + "&arg3=" + content + 
