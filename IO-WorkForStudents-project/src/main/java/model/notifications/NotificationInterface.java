...
public class NotificationInterface extends Interface {
    ...
    public ArrayList<Notification> getNotifications(String userID) {
            ...
            WHERE users.id_user = ?""";
                            ...
                            while (results.next()) {
                                    ...
                                    String IDOffer = results.getString("id_offer");

                                    Notification notification = new Notification(notificationID, messageType, employeeLogin, offerTitle, IDUser,IDOffer);
            ...
            WHERE offers.id_empl = ?""";
                            ...
                            while (results.next()) {
                                    ...
                                    Notification notification = new Notification(notificationID, messageType, studentLogin, offerTitle, IDUser, IDOffer);
                                    notifications.add(notification);
                            }
                    }
                    else {
    ...
    public Notification getNotification(String id_notif, String userType) {
        try {
            if (userType.equals("student")) {
                String query = "SELECT * FROM mailbox_student WHERE id_box_stud = ?";
                PreparedStatement preparedStatement = connection.prepareStatement(query);
                preparedStatement.setString(1, id_notif);
                ResultSet results = preparedStatement.executeQuery();

                if (results.next()) {
                    String id_offer = results.getString("id_offer");
                    String mess_type = results.getString("mess_type");
                    String id_stud = results.getString("id_stud");
    ...
    public void createNotification(Notification notif,String userType, String action) {
        try {
            if(userType.equals("student")){
                ...
                insertStatement.setString(2, notif.getOfferID());
                insertStatement.setString(3, notif.getUserID());
                ...
                PreparedStatement insertStatement = connection.prepareStatement(insertQuery);
                insertStatement.setString(1, String.valueOf(newMax));
                insertStatement.setString(2, notif.getUserID());
                insertStatement.setString(3, notif.getOfferID());
    ...
    public void deleteNotification(String id_notif, String userType) {
        try {
            if(userType.equals("student")){
            String insertQuery = "DELETE FROM mailbox_student WHERE id_box_stud = ?";
            PreparedStatement insertStatement = connection.prepareStatement(insertQuery);
            insertStatement.setString(1, String.valueOf(id_notif));
            insertStatement.executeUpdate();
            } else if (userType.equals("employer")) {
